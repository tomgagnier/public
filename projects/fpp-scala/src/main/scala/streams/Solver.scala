...
trait Solver extends GameDef {
   ...
   */
  def newNeighborsOnly(neighborsWithHistory: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] =
    for {
      (neighbor, history) <- neighborsWithHistory
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    ...
    if (initial.isEmpty) Stream.empty
      ...
      initial #::: from(candidates, explored ++ (candidates.map(_._1)))
    }
  }
