...
object Anagrams {
  ...
  def wordOccurrences(word: Word): Occurrences =
    word.toLowerCase.toList.groupBy(c => c).toList.
      map { case (char, chars) => (char, chars.size) }.
    	  sortBy { case (char, _) => char }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    wordOccurrences(s.foldLeft(""){(a, b) => a + b})
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def collect(occurrences: Occurrences, result: List[Occurrences]): List[Occurrences] =
      if (occurrences.isEmpty) List() :: result
      else for {
        occurrence <- collect(occurrences.tail, result)
        (char, count) = occurrences.head
        c <- 0 to count
      } yield { if (c == 0) occurrence else (char, c) :: occurrence }

    collect(occurrences, List())
  }
   ...
   */
  def subtract(left: Occurrences, right: Occurrences): Occurrences = {
    val subtrahend = right.toMap.withDefaultValue(0)
    for {
      (char, count) <- left
      if (count != subtrahend(char))
    } yield (char, (count - subtrahend(char)))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(List())
        ...
        remainingWords <- anagrams(subtract(occurrences, combination))
